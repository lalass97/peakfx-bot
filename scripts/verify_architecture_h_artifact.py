...
CONFIGS={"H01","H02","H03","H04"}; WINDOWS={"2020_2021","2021_2022","2022_2023","2023_2024","2024_2025"}; EXPECTED={(c,w) for c in CONFIGS for w in WINDOWS}
def sha(p):
 h=hashlib.sha256()
 with p.open('rb') as f:
...
def main():
 ...
 mp=r/'architecture_h_manifest.json'
 ...
 for x in runs:
  ...
  if pair in seen:err.append(f'Duplicate cell {c}/{w}')
  seen.add(pair);src.add(str(x.get('source_sha256','')));binh.add(str(x.get('binary_sha256','')));spec.add(str(x.get('spec_sha256','')))
  if c not in CONFIGS or w not in WINDOWS:err.append(f'Unexpected cell {c}/{w}')
  if x.get('oos_locked') is not True:err.append(f'OOS unlocked {c}/{w}')
  if str(x.get('model'))!='real_ticks':err.append(f'Wrong model {c}/{w}')
  e=x.get('execution',{})
  if float(e.get('bars',0))<=0 or float(e.get('ticks',0))<=0:err.append(f'Empty execution {c}/{w}')
  if abs(float(e.get('initial_deposit',0))-10000.0)>0.01:err.append(f'Wrong deposit {c}/{w}')
 ...
 print('Verification PASSED: Architecture H 20/20 artifact verified.');return 0
