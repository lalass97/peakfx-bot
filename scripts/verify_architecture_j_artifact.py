...
CONFIGS={"J01","J02","J03","J04"}; WINDOWS={"2020_2021","2021_2022","2022_2023","2023_2024","2024_2025"}; EXPECTED={(c,w) for c in CONFIGS for w in WINDOWS}
def sha(p):
 h=hashlib.sha256();
 with p.open('rb') as f:
...
def main():
 ...
 mp=r/'architecture_j_manifest.json'
 ...
 for x in runs:
  c,w=str(x.get('configuration','')),str(x.get('window',''));seen.add((c,w));src.add(str(x.get('source_sha256','')));binh.add(str(x.get('binary_sha256','')));spec.add(str(x.get('spec_sha256','')))
 ...
 print('Verification PASSED: Architecture J 20/20 artifact verified.');return 0
