...
EXPECTED_CONFIGS={"F01","F02","F03","F04"}
...
def main()->int:
    p=argparse.ArgumentParser();p.add_argument('artifact_root',type=Path);p.add_argument('--output',type=Path);a=p.parse_args();root=a.artifact_root.resolve();errors=[];checks=[]
    mp=root/'architecture_f_manifest.json'
    if not mp.is_file():print(f'Missing manifest: {mp}',file=sys.stderr);return 2
    manifest=load_json(mp);runs=manifest.get('runs',[])
    if len(runs)!=20:errors.append(f'Expected 20 manifest runs, found {len(runs)}')
    seen=set();src=set();binh=set();spec=set()
    for run in runs:
        cfg=str(run.get('configuration',''));win=str(run.get('window',''));pair=(cfg,win)
        ...
        if float(ex.get('bars',0))<=0 or float(ex.get('ticks',0))<=0:errors.append(f'Empty execution for {cfg}/{win}')
        if abs(float(ex.get('initial_deposit',0))-10000.0)>0.01:errors.append(f'Wrong deposit for {cfg}/{win}')
        report=Path(str(run.get('report','')))
        if not report.is_absolute():
            cand=[root/report,root/cfg/win/report.name];report=next((x for x in cand if x.is_file()),cand[-1])
        else:
            report=root/cfg/win/report.name
        if not report.is_file():errors.append(f'Missing report for {cfg}/{win}: {report}');continue
        rh=sha256(report)
    ...
    copies=[(frozen/'PeakFX_EURUSD_ARCH_F_LONDON_OPENING_RANGE.mq5',src,'source'),(frozen/'PeakFX_EURUSD_ARCH_F_LONDON_OPENING_RANGE.ex5',binh,'binary'),(frozen/'ARCHITECTURE_F_FROZEN_BASELINE_SPEC.md',spec,'specification')]
    ...
    result={'verified':not errors,'expected_run_count':20,'observed_run_count':len(runs),'matrix_complete':seen==EXPECTED_RUNS,'checks':checks,'errors':errors}
    out=a.output or root/'independent_verification.json';out.write_text(json.dumps(result,indent=2),encoding='utf-8')
    if errors:
        for e in errors:print(f'ERROR: {e}',file=sys.stderr)
        return 1
    print('Verification PASSED: Architecture F 20/20 artifact verified.');return 0
