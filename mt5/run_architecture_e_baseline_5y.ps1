...
$results=Join-Path $RepoRoot 'artifacts/architecture-e-baseline-5y'
...
$source=Join-Path $RepoRoot 'mt5/PeakFX_EURUSD_ARCH_E_DAILY_FOLLOWTHROUGH.mq5'
$spec=Join-Path $RepoRoot 'docs/ARCHITECTURE_E_FROZEN_BASELINE_SPEC.md'
$verifier=Join-Path $RepoRoot 'scripts/verify_architecture_e_artifact.py'
...
$configs=@(
  @{id='E01';body='0.55';target='1.00'},@{id='E02';body='0.65';target='1.00'},
  @{id='E03';body='0.55';target='1.50'},@{id='E04';body='0.65';target='1.50'}
)
$windows=@(
...
foreach($cfg in $configs){
  $setName="architecture_e_$($cfg.id.ToLower()).set";$setPath=Join-Path $testerProfiles $setName
  $setLines=@(
    ...
    (Set-Line 'InpBodyFraction' $cfg.body $cfg.body '0.05' $cfg.body),(Set-Line 'InpTargetR' $cfg.target $cfg.target '0.10' $cfg.target),
    (Set-Line 'InpStopBufferFraction' '0.10' '0.10' '0.01' '0.10'),(Set-Line 'InpMaxSpreadPips' '2.0' '2.0' '0.1' '2.0'),
    (Set-Line 'InpDeviationPoints' '10' '10' '1' '10'),(Set-Line 'InpMagic' '26080705' '26080705' '1' '26080705')
  )
  ...
  foreach($w in $windows){
    ...
    $m=[ordered]@{architecture='E';configuration=$cfg.id;window=$w.id;start=$w.from;end=$w.to;body_fraction=[double]$cfg.body;target_r=[double]$cfg.target;parameter_set=$setName;parameter_set_sha256=$setHash;spec_sha256=$specHash;symbol='EURUSD';timeframe='M15';model='real_ticks';deposit=$Deposit;leverage=$Leverage;source_sha256=$sourceHash;binary_sha256=$binaryHash;report=$report;report_sha256=$reportHash;terminal_exit_code=$tp.ExitCode;oos_locked=$true;execution=$execution;terminal_data_root=$dataRoot}
...
$manifest=[ordered]@{protocol='Architecture E frozen baseline';expected_run_count=20;completed_run_count=$runs.Count;source_sha256=$sourceHash;binary_sha256=$binaryHash;spec_sha256=$specHash;compile_log=$compileLog;compile_exit_code=$cp.ExitCode;terminal_data_root=$dataRoot;configurations=$configs;windows=$windows;runs=$runs;oos_locked=$true;reserved_oos_not_tested=$true;generated_at=(Get-Date).ToString('o')}
$manifest|ConvertTo-Json -Depth 9|Set-Content (Join-Path $results 'architecture_e_manifest.json') -Encoding UTF8
Write-Host 'Architecture E baseline complete: 20 non-empty real-tick reports.'
