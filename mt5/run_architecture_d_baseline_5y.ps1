...
$results=Join-Path $RepoRoot 'artifacts/architecture-d-baseline-5y'
...
$source=Join-Path $RepoRoot 'mt5/PeakFX_EURUSD_ARCH_D_IMPULSE_PULLBACK.mq5'
$spec=Join-Path $RepoRoot 'docs/ARCHITECTURE_D_FROZEN_BASELINE_SPEC.md'
$verifier=Join-Path $RepoRoot 'scripts/verify_architecture_d_artifact.py'
...
$configs=@(
  @{id='D01';imp='1.00';pb='0.382'},@{id='D02';imp='1.25';pb='0.382'},
  @{id='D03';imp='1.00';pb='0.500'},@{id='D04';imp='1.25';pb='0.500'}
)
$windows=@(
...
foreach($cfg in $configs){
  $setName="architecture_d_$($cfg.id.ToLower()).set";$setPath=Join-Path $testerProfiles $setName
  $setLines=@(
    ...
    (Set-Line 'InpImpulseRangeMultiple' $cfg.imp $cfg.imp '0.05' $cfg.imp),(Set-Line 'InpPullbackFraction' $cfg.pb $cfg.pb '0.01' $cfg.pb),
    (Set-Line 'InpEfficiencyMin' '0.60' '0.60' '0.05' '0.60'),(Set-Line 'InpStopBufferMultiple' '0.15' '0.15' '0.05' '0.15'),
    (Set-Line 'InpTargetR' '1.50' '1.50' '0.10' '1.50'),(Set-Line 'InpMaxSpreadPips' '2.0' '2.0' '0.1' '2.0'),
    (Set-Line 'InpDeviationPoints' '10' '10' '1' '10'),(Set-Line 'InpMagic' '26080704' '26080704' '1' '26080704')
  )
  ...
  foreach($w in $windows){
    ...
    $m=[ordered]@{architecture='D';configuration=$cfg.id;window=$w.id;start=$w.from;end=$w.to;impulse_range_multiple=[double]$cfg.imp;pullback_fraction=[double]$cfg.pb;parameter_set=$setName;parameter_set_sha256=$setHash;spec_sha256=$specHash;symbol='EURUSD';timeframe='H1';model='real_ticks';deposit=$Deposit;leverage=$Leverage;source_sha256=$sourceHash;binary_sha256=$binaryHash;report=$report;report_sha256=$reportHash;terminal_exit_code=$tp.ExitCode;oos_locked=$true;execution=$execution;terminal_data_root=$dataRoot}
...
$manifest=[ordered]@{protocol='Architecture D frozen baseline';expected_run_count=20;completed_run_count=$runs.Count;source_sha256=$sourceHash;binary_sha256=$binaryHash;spec_sha256=$specHash;compile_log=$compileLog;compile_exit_code=$cp.ExitCode;terminal_data_root=$dataRoot;configurations=$configs;windows=$windows;runs=$runs;oos_locked=$true;reserved_oos_not_tested=$true;generated_at=(Get-Date).ToString('o')}
$manifest|ConvertTo-Json -Depth 9|Set-Content (Join-Path $results 'architecture_d_manifest.json') -Encoding UTF8
Write-Host 'Architecture D baseline complete: 20 non-empty real-tick reports.'
