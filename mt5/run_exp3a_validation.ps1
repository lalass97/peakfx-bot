...
# Resolve the active MT5 data folder from the verified EXP1 source, then deploy
# EXP3A into the same PeakFX subfolder referenced by the tester configuration.
$ExpertsRoot = Split-Path -Parent $CompiledCleanExp1Source
if (-not (Test-Path $ExpertsRoot -PathType Container)) { throw "Active MT5 Experts root not found: $ExpertsRoot" }
...
"@
    ...
    $foundReport = Find-Mt5Report -ReportStem $reportStem -StartedAt $startedAt -StageDirectory $stageDir
    if (-not $foundReport) {
    ...
    [ordered]@{
        ...
        source_path=$CandidateSource; deployed_source_path=$DeployedSource
        deployed_binary_path=$DeployedBinary; mt5_data_folder=$Mt5DataFolder
        original_report_path=$foundReport.FullName; report_path=$reportPath
