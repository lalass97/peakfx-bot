...
#property description "PeakFX EURUSD H1 trend-following EA with ADX filter - demo only"
...
input long MagicNumber = 26073002;
input bool EnableTelemetry = true;
input string TelemetryFolder = "PeakFX";
input string TelemetryFile = "peakfx_adx_events.csv";
...
void LogEvent(const string eventName, const ulong ticket = 0, const string message = "")
{
   ...
   int handle = FileOpen(TelemetryPath(), FILE_READ | FILE_WRITE | FILE_CSV | FILE_ANSI | FILE_COMMON | FILE_SHARE_READ, ',');
   if(handle == INVALID_HANDLE){ Print("Telemetry open failed. Error=", GetLastError()); ResetLastError(); return; }
   if(FileSize(handle) == 0) FileWrite(handle, "time", "event", "symbol", "magic", "ticket", "message");
   FileSeek(handle, 0, SEEK_END);
   FileWrite(handle, UtcTimestamp(), eventName, InpSymbol, (string)MagicNumber, (string)ticket, CleanTelemetryMessage(message));
   FileFlush(handle);
   FileClose(handle);
}
...
void RefreshRiskState()
{
   ...
   double equity = AccountInfoDouble(ACCOUNT_EQUITY);
   bool changed = false;
   if(today != dayAnchor){ dayAnchor = today; dayStartEquity = equity; tradesToday = CountTodayTrades(); changed = true; }
   if(monday != weekAnchor){ weekAnchor = monday; weekStartEquity = equity; changed = true; }
   if(equity > equityHighWater){ equityHighWater = equity; changed = true; }
...
bool PortfolioRiskAllowsTrading()
{
   ...
   if(dailyLoss >= MaxDailyLossPercent){ LogEvent("daily_lock", 0, StringFormat("daily_loss_pct=%.3f", dailyLoss)); return false; }
   double weeklyLoss = LossPercent(weekStartEquity);
   if(weeklyLoss >= MaxWeeklyLossPercent){ LogEvent("weekly_lock", 0, StringFormat("weekly_loss_pct=%.3f", weeklyLoss)); return false; }
   double highWaterLoss = LossPercent(equityHighWater);
   if(highWaterLoss >= MaxHighWaterDrawdownPercent){ LogEvent("drawdown_lock", 0, StringFormat("drawdown_pct=%.3f", highWaterLoss)); return false; }
   return true;
}
...
bool ReadIndicators(double &fast1, double &fast2, double &slow1, double &slow2,
                    double &trend1, double &trend6, double &atr1, double &adx1, double &close1)
{
   double fast[], slow[], trend[], atr[], adx[], closeBuf[];
   ArrayResize(fast, 3); ArrayResize(slow, 3); ArrayResize(trend, 7);
   ArrayResize(atr, 3); ArrayResize(adx, 3); ArrayResize(closeBuf, 3);
   ArraySetAsSeries(fast, true); ArraySetAsSeries(slow, true); ArraySetAsSeries(trend, true);
   ArraySetAsSeries(atr, true); ArraySetAsSeries(adx, true); ArraySetAsSeries(closeBuf, true);
   ...
   if(CopyBuffer(adxHandle, 0, 0, 3, adx) < 3) return false;
   if(CopyClose(InpSymbol, InpTimeframe, 0, 3, closeBuf) < 3) return false;
   fast1 = fast[1]; fast2 = fast[2]; slow1 = slow[1]; slow2 = slow[2];
   trend1 = trend[1]; trend6 = trend[6]; atr1 = atr[1]; adx1 = adx[1]; close1 = closeBuf[1];
   return atr1 > 0.0 && adx1 >= 0.0;
}
...
void EvaluateSignal()
{
   ...
   if(tradesToday >= MaxTradesPerDay){ LogEvent("risk_block", 0, "maximum daily trade count reached"); return; }
   if(HasOpenPosition()){ LogEvent("risk_block", 0, "PeakFX position already open"); return; }
   double spreadPoints = CurrentSpreadPoints();
   if(spreadPoints < 0.0 || spreadPoints > MaxSpreadPoints){ LogEvent("spread_block", 0, StringFormat("spread_points=%.1f", spreadPoints)); return; }
   if(!CooldownComplete()){ LogEvent("cooldown_block", 0, "cooldown bars not complete"); return; }

   double fast1, fast2, slow1, slow2, trend1, trend6, atr1, adx1, close1;
   if(!ReadIndicators(fast1, fast2, slow1, slow2, trend1, trend6, atr1, adx1, close1))
   { LogEvent("risk_block", 0, "indicator data unavailable"); return; }
   ...
   MqlTick tick;
   if(!SymbolInfoTick(InpSymbol, tick)){ LogEvent("order_rejected", 0, "symbol tick unavailable"); return; }
   double stopDistance = atr1 * ATRStopMultiplier;
   double volume = CalculateVolume(stopDistance);
   if(volume <= 0.0){ LogEvent("volume_block", 0, "calculated volume below broker minimum or invalid"); return; }
   ...
   if(!StopsValid(entry, sl, tp)){ LogEvent("order_rejected", 0, "SL or TP violates broker minimum stop distance"); return; }
   if(!MarginAvailable(orderType, volume, entry)){ LogEvent("margin_block", 0, StringFormat("volume=%.2f", volume)); return; }

   LogEvent(longSignal ? "signal_long" : "signal_short", 0, StringFormat("adx=%.2f atr=%.5f spread_points=%.1f", adx1, atr1, spreadPoints));
   if(longSignal) placed = trade.Buy(volume, InpSymbol, 0.0, sl, tp, "PeakFX EURUSD H1 ADX long");
   else placed = trade.Sell(volume, InpSymbol, 0.0, sl, tp, "PeakFX EURUSD H1 ADX short");
   ...
   if(placed)
   {
      ...
      SaveState();
      LogEvent("order_filled", trade.ResultDeal(), StringFormat("order=%I64u volume=%.2f result=%s", trade.ResultOrder(), volume, trade.ResultRetcodeDescription()));
   }
...
int OnInit()
{
   ...
   if(FastEMA <= 0 || SlowEMA <= FastEMA || TrendEMA <= SlowEMA) return INIT_PARAMETERS_INCORRECT;
   if(ATRPeriod <= 0 || ADXPeriod <= 0 || MinADX < 0.0) return INIT_PARAMETERS_INCORRECT;
   ...
   adxHandle = iADX(InpSymbol, InpTimeframe, ADXPeriod);
   ...
   LogEvent("startup", 0, StringFormat("version=1.30 demo_only=%s min_adx=%.2f trades_today=%d equity=%.2f", DemoOnly ? "true" : "false", MinADX, tradesToday, AccountInfoDouble(ACCOUNT_EQUITY)));
...
{
   RefreshRiskState();
   LogEvent("heartbeat", 0, StringFormat("equity=%.2f daily_loss_pct=%.3f weekly_loss_pct=%.3f trades_today=%d open_position=%s", AccountInfoDouble(ACCOUNT_EQUITY), LossPercent(dayStartEquity), LossPercent(weekStartEquity), tradesToday, HasOpenPosition() ? "true" : "false"));
}
...
void OnTradeTransaction(const MqlTradeTransaction &trans, const MqlTradeRequest &request, const MqlTradeResult &result)
{
   ...
   if(entryType == DEAL_ENTRY_IN || entryType == DEAL_ENTRY_INOUT) LogEvent("position_opened", trans.deal, StringFormat("price=%.5f volume=%.2f", price, volume));
   if(entryType == DEAL_ENTRY_OUT || entryType == DEAL_ENTRY_OUT_BY || entryType == DEAL_ENTRY_INOUT) LogEvent("position_closed", trans.deal, StringFormat("price=%.5f volume=%.2f net_profit=%.2f", price, volume, profit));
}
