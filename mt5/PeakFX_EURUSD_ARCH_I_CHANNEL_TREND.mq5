...
CTrade trade;
int h4ema=INVALID_HANDLE,h1atr=INVALID_HANDLE; datetime lastH1=0;
...
void Evaluate(){if(OurPos()||!SpreadOK())return;MqlDateTime d;TimeToStruct(TimeGMT(),d);if(d.day_of_week==0||d.day_of_week==6||(d.day_of_week==5&&d.hour>=18))return;int tr;if(!H4Trend(tr))return;double hi,lo;if(!Channel(InpEntryChannel,2,hi,lo))return;double o=iOpen(InpSymbol,PERIOD_H1,1),c=iClose(InpSymbol,PERIOD_H1,1);int dir=0;if(tr>0&&c>hi&&c>o)dir=1;else if(tr<0&&c<lo&&c<o)dir=-1;if(dir==0)return;double atr;if(!CopyOne(h1atr,1,atr)||atr<=0)return;MqlTick t;if(!SymbolInfoTick(InpSymbol,t))return;int digits=(int)SymbolInfoInteger(InpSymbol,SYMBOL_DIGITS);trade.SetExpertMagicNumber(InpMagic);trade.SetDeviationInPoints(InpDeviationPoints);if(dir>0){double e=t.ask,sl=NormalizeDouble(e-InpAtrStopMultiple*atr,digits),risk=e-sl,tp=InpFixed2R?NormalizeDouble(e+2.0*risk,digits):0;double v=VolForRisk(ORDER_TYPE_BUY,e,sl);if(v>0)trade.Buy(v,InpSymbol,0,sl,tp,"ARCH_I");}else{double e=t.bid,sl=NormalizeDouble(e+InpAtrStopMultiple*atr,digits),risk=sl-e,tp=InpFixed2R?NormalizeDouble(e-2.0*risk,digits):0;double v=VolForRisk(ORDER_TYPE_SELL,e,sl);if(v>0)trade.Sell(v,InpSymbol,0,sl,tp,"ARCH_I");}}
int OnInit(){if(!SymbolSelect(InpSymbol,true))return INIT_FAILED;h4ema=iMA(InpSymbol,PERIOD_H4,200,0,MODE_EMA,PRICE_CLOSE);h1atr=iATR(InpSymbol,PERIOD_H1,14);if(h4ema==INVALID_HANDLE||h1atr==INVALID_HANDLE)return INIT_FAILED;trade.SetExpertMagicNumber(InpMagic);return INIT_SUCCEEDED;}
...
void OnTick(){ForceFriday();datetime b=iTime(InpSymbol,PERIOD_H1,0);if(b<=0||b==lastH1)return;lastH1=b;ManageChannelExit();Evaluate();}
