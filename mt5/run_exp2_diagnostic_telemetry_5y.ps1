...
$ResultsRoot = Join-Path $RepoRoot "artifacts/mt5-exp2-diagnostic-telemetry-5y"
...
$TelemetryBuilder = Join-Path $RepoRoot "research/build_exp2_diagnostic_telemetry.py"
$MetaEditor = Join-Path $MetaTraderRoot "metaeditor64.exe"
$Terminal = Join-Path $MetaTraderRoot "terminal64.exe"

foreach ($required in @($MetaEditor,$Terminal,$CompiledCleanExp1Source,$Exp2Builder,$TelemetryBuilder)) {
...
function Compile-Candidate {
    param([string]$SourcePath,[string]$ExpertsDir)
    ...
    if (-not (Test-Path $binary -PathType Leaf)) { throw "Compiled EX5 not found: $binary" }
...
if ($LASTEXITCODE -ne 0 -or -not (Test-Path $diagnosticSource)) { throw "EXP2 telemetry generation failed" }
...
foreach ($window in $windows) {
    ...
    $config = @"
[Tester]
Expert=$ExpertSubdir\$expertName
Symbol=EURUSD
Period=H1
Model=4
ExecutionMode=0
