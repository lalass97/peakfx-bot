...
$ResultsRoot = Join-Path $RepoRoot "artifacts/mt5-exp6a"
...
if ($LASTEXITCODE -ne 0 -or -not (Test-Path $CandidateSource)) { throw "EXP6A candidate generation failed" }
...
"@
    ...
    [ordered]@{
        candidate_id='peakfx_exp6a_rising_er_v1_47'
        parent='peakfx_exp3a_er20_035_v1_46'
        isolated_change='ER gate passes when ER >= 0.35 OR ER >= 0.30 and rises by at least 0.05 versus prior completed H1 bar; all other logic unchanged'
...
Write-Host "EXP6A smoke and fixed 12-month screen completed. OOS remains locked."
