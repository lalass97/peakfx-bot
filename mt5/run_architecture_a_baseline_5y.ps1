...
foreach($cfg in $configs){
 $setName="architecture_a_$($cfg.id.ToLower()).set"
 $setPath=Join-Path $testerProfiles $setName
 $setText="InpBreakoutBufferAtr=$($cfg.buffer)||$($cfg.buffer)||0.01||0.50||N`r`nInpTargetR=$($cfg.target)||$($cfg.target)||0.05||3.00||N`r`nInpRequireH4EmaSlope=false||false||0||true||N`r`n"
 ...
 foreach($w in $windows){
  ...
  Stop-Mt5;$started=Get-Date;Write-Host "Running $($cfg.id) $($w.id) using $setName..."
