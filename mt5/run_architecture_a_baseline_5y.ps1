...
$results=Join-Path $RepoRoot 'artifacts/architecture-a-baseline-5y'
...
foreach($root in $terminalRoots){
  if(Test-Path (Join-Path $root.FullName 'MQL5\Experts')){$dataRoot=$root.FullName;break}
}
...
if($compileText-notmatch '(?i)\b0\s+errors?\s*,\s*0\s+warnings?\b'){
  Get-Content $compileLog|ForEach-Object{Write-Host $_};throw 'Compile gate failed'
}
...
$configs=@(
 @{id='A01';buffer='0.05';target='1.25'},
 @{id='A02';buffer='0.05';target='1.50'},
 @{id='A03';buffer='0.10';target='1.25'},
 @{id='A04';buffer='0.10';target='1.50'},
 @{id='A05';buffer='0.15';target='1.25'},
 @{id='A06';buffer='0.15';target='1.50'}
)
$windows=@(
...
foreach($cfg in $configs){
 foreach($w in $windows){
  ...
  $stem="arch_a_$($cfg.id.ToLower())_$($w.id)_report";$ini=Join-Path $stage "$stem.ini"
  $text="[Tester]`r`nExpert=PeakFX\PeakFX_EURUSD_ARCH_A_SESSION_BREAKOUT`r`nExpertParameters=InpBreakoutBufferAtr=$($cfg.buffer)||InpTargetR=$($cfg.target)||InpRequireH4EmaSlope=false`r`nSymbol=EURUSD`r`nPeriod=M15`r`nModel=4`r`nExecutionMode=0`r`nOptimization=0`r`nFromDate=$($w.from)`r`nToDate=$($w.to)`r`nForwardMode=0`r`nDeposit=$Deposit`r`nCurrency=USD`r`nLeverage=$lev`r`nReport=$stem`r`nReplaceReport=1`r`nShutdownTerminal=1`r`nVisual=0`r`n"
  Set-Content $ini $text -Encoding ASCII
  Stop-Mt5;$started=Get-Date;Write-Host "Running $($cfg.id) $($w.id)..."
  $tp=Start-Process -FilePath $terminal -WorkingDirectory $MetaTraderRoot -ArgumentList @("/config:$ini") -Wait -PassThru
  $found=Find-Report $stem $started $stage;if(-not$found){throw "No report for $($cfg.id) $($w.id)"}
  $dest=Join-Path $stage $found.Name;if($found.FullName-ine$dest){Copy-Item $found.FullName $dest -Force}
  $m=[ordered]@{architecture='A';configuration=$cfg.id;buffer_atr=[double]$cfg.buffer;target_r=[double]$cfg.target;stop_atr_floor=1.0;h4_slope_revision=$false;window=$w.id;start=$w.from;end=$w.to;symbol='EURUSD';timeframe='M15';model='real_ticks';deposit=$Deposit;leverage=$Leverage;source_sha256=$sourceHash;binary_sha256=$binaryHash;report=$dest;terminal_exit_code=$tp.ExitCode;oos_locked=$true}
  $m|ConvertTo-Json -Depth 5|Set-Content (Join-Path $stage 'run_metadata.json') -Encoding UTF8;$runs+=$m
 }
}
$manifest=[ordered]@{protocol='Architecture A frozen baseline';source_sha256=$sourceHash;binary_sha256=$binaryHash;compile_log=$compileLog;compile_exit_code=$cp.ExitCode;configurations=$configs;windows=$windows;runs=$runs;oos_locked=$true;reserved_oos_not_tested=$true;predeclared_revision='A-R1 H4 EMA20 slope';generated_at=(Get-Date).ToString('o')}
$manifest|ConvertTo-Json -Depth 8|Set-Content (Join-Path $results 'architecture_a_manifest.json') -Encoding UTF8
Write-Host 'Architecture A baseline batch complete. OOS remains locked.'
